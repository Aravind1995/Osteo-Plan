...
/* OSTEOPLAN_LANDSCAPE_PROMPT_SNIPPET_v1 */

/* OSTEOPLAN_LANDSCAPE_PROMPT_SNIPPET_v1
   Shows a single, dismissible dialog on small portrait devices
   prompting the user to view in landscape. Appended automatically.
*/
(function(){
  function showLandscapePrompt(){
    var mq = window.matchMedia && window.matchMedia("(max-width:700px) and (orientation: portrait)");
    if (!mq) return;
    if (!mq.matches) { var ex = document.getElementById('landscapePrompt'); if (ex) ex.remove(); return; }
    if (localStorage.getItem('osteo_hide_landscape_prompt')) return;
    if (document.getElementById('landscapePrompt')) return;
    var el = document.createElement('div');
    el.id = 'landscapePrompt';
    ...
    el.innerHTML = '<div style="background:#0f1420;color:#fff;padding:18px;border-radius:12px;border:1px solid rgba(255,255,255,.06);max-width:520px;width:92%;text-align:center;font-size:16px;">' +
                   'For the best experience, please rotate your device to <strong>landscape</strong>.' +
                   '<div style="margin-top:14px;display:flex;gap:10px;justify-content:center;">' +
                   '<button id="landscapeOk" style="padding:8px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);color:#fff;">OK</button>' +
                   '<button id="landscapeHide" style="padding:8px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.03);color:#fff;">Don\\'t show again</button>' +
                   '</div></div>';
    document.body.appendChild(el);
    document.getElementById('landscapeOk').addEventListener('click', function(){ el.remove(); });
    document.getElementById('landscapeHide').addEventListener('click', function(){ localStorage.setItem('osteo_hide_landscape_prompt','1'); el.remove(); });
  }
  window.addEventListener('resize', showLandscapePrompt, {passive:true});
  window.addEventListener('orientationchange', showLandscapePrompt, {passive:true});
  document.addEventListener('DOMContentLoaded', showLandscapePrompt);
})();
